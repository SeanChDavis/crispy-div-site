export default function LinkButton({text, url, style = "dark", fill = true, customClasses, target = "_self", refer = true}: {
    ...
    let baseClasses = "link-button inline-block border-2 text-sm font-semibold rounded-full px-4 py-2 cursor-pointer tracking-wide transition-all duration-300 ease-in-out active:scale-[.97] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-600";
    let combinedClasses = "";

    if (style === "dark") {
        baseClasses += " border-zinc-950 dark:border-white hover:bg-transparent dark:hover:border-white focus:ring-offset-white dark:focus:ring-offset-zinc-950";
        if (fill) {
